...
// ---------- CCITT-ENCODED IMAGE DETECTION ----------
// pdf.js - what ARML's in-app PDF viewer is built on - has a long-
// documented, still-unfixed rendering bug with CCITT-encoded (fax
// Group 3/4) images: affected pages render as a barely-visible washed-
// out ghost of the real content. It isn't a bug in the files themselves
// (independently verified - the same image data renders correctly
// through other tools), and it isn't something ARML's own code can work
// around - it's specific to pdf.js's handling of this one codec. 31 of
// ARML's original resources shipped this way (most scanned documents
// export as CCITT by default) and were individually converted to a
// codec pdf.js has no known issues with. This check is what stops that
// from being a one-time fix that quietly wears off: a scanned document
// added later that happens to export as CCITT would otherwise ship
// silently broken again, the same way this exact bug already reached
// production unnoticed more than once before anyone realized it was a
// pattern rather than a one-off.
//
// See detect-ccitt-images.js for why this is a detector, not an
// auto-fixer.
(() => {
  try {
    const out = execFileSync("node", [path.join(__dirname, "detect-ccitt-images.js")], {
      encoding: "utf8",
      maxBuffer: 10 * 1024 * 1024
    });
    const parsed = JSON.parse(out);
    if (!parsed.available) {
      console.warn(
        "\n  Note: pdf-lib isn't installed, so CCITT-image detection is off.\n" +
        "  Run 'npm install' in ARM-Builder to enable it.\n"
      );
      return;
    }
    if (parsed.offenders.length > 0) {
      console.warn(
        "\n" +
        "  \u26a0\u26a0\u26a0 CCITT-ENCODED IMAGE(S) FOUND - WILL RENDER BROKEN IN THE APP \u26a0\u26a0\u26a0\n" +
        "  " + "=".repeat(68) + "\n" +
        "  The file(s) below use an image format pdf.js can't display\n" +
        "  correctly (see this comment in build-data.js for the full story).\n" +
        "  The build is NOT blocked, but these WILL look washed-out and\n" +
        "  barely readable in ARML until fixed:\n\n" +
        parsed.offenders.map(o => `    - ${o.file}  (page${o.pages.length > 1 ? "s" : ""} ${o.pages.join(", ")})`).join("\n") +
        "\n\n  Ask Claude (or see the fix already applied to 31 other files in\n" +
        "  this same project) to convert the flagged image(s) to a standard\n" +
        "  codec - same pixels, different compression, no visible change\n" +
        "  except that it actually renders.\n" +
        "  " + "=".repeat(68) + "\n"
      );
    }
  } catch (err) {
    console.warn("\n  Note: CCITT-image detection failed (" + err.message + ").\n");
  }
})();
...
const shellFiles = [
  ...
  ...listIcons(),
  // pdf.js, vendored for offline in-app PDF viewing/filling (see
  // vendor/pdfjs/ - annotation-layer.css has the extraction/upgrade
  // notes). Listed by hand rather than auto-discovered like icons/,
  // since there's no reason to blindly ship every file npm happens to
  // put in a future pdfjs-dist version - standard_fonts/ in particular
  // is a large directory ARML only needs a few files from at most, and
  // is intentionally left out of the shell precache; it's fetched
  // on-demand by pdf.js itself only for the rare PDF that doesn't embed
  // its own fonts.
