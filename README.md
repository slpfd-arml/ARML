...
| `ARM-Builder/` | The build script (`build-data.js`) and the source workbook (`New\\\_ARM\\\_Library.xlsx`) that generates `data.js` |
...
**Editing the workbook directly** (`ARM-Builder/New\\\_ARM\\\_Library.xlsx` in Excel) still works and is sometimes the right tool — bulk edits across many rows, or fixing something ARML Editor's form doesn't cover. If you go this route:
...
- If you have a recent copy of `New\\\_ARM\\\_Library.xlsx` (a backup, or an earlier version still in `node\\\_modules`-adjacent history if you're using git), you can manually re-add the row and rebuild.
...
- The app does a fuzzy filename match, so minor spelling differences are OK (`Fact\\\_Sheet.pdf` and `Fact Sheet.pdf` are treated as the same)
...
5. **Make sure they know:** ARML Editor is the normal way to add, edit, or delete resources — the workbook is what it edits, not something they need to touch by hand day to day. The app code is stable and rarely needs changes.
...
2. The data flow is: `New\\\_ARM\\\_Library.xlsx` → `build-data.js` → `data.js` → `app.js` renders it
