...
//   ""                                  -> []
//   "AUDIT.pdf"                         -> [{label:"AUDIT.pdf", path:"...", fillable:false}]
//   "Depression Screener|PHQ-9.pdf"     -> [{label:"...", path:"...", fillable:false}]
//   "A.pdf; Label|B.pdf"                -> two entries, split on ";"
//
// An optional trailing "|fillable" segment marks a SPECIFIC file as a real
// fillable form, not just a reference PDF - e.g.
// "Metro Mobility Application|Metro_Mobility_Application.pdf|fillable".
// This exists because Safari's own built-in PDF viewer frequently renders
// AcroForm fields as a flat, non-interactive image (a well-documented,
// longstanding limitation, not something fixable by how the link is
// built) - so fillable-flagged files always route through the device's
// native "Open in..." share sheet instead of Safari's inline viewer,
// letting the person pick a PDF app that actually handles forms, and
// leaving a real copy in Files for next time.
//
// forceFillable applies that same treatment to EVERY file from a sheet,
// with no per-row marker needed - used for Release of Information, where
// the whole category is fillable forms by definition.
function parseFiles(raw, context, forceFillable) {
  if (!raw) return [];
  return String(raw)
    .split(";")
    .map(s => cleanText(s))
    ...
    .map(entry => {
      ...
      const isFillableMarked = parts.length > 1 && parts[parts.length - 1].toLowerCase() === "fillable";
      if (isFillableMarked) parts = parts.slice(0, -1);

      const label = parts[0];
      const filename = parts.length === 1 ? parts[0] : parts[parts.length - 1];
      const resolved = resolveFile(filename, context);
      return resolved ? { label, path: resolved, fillable: Boolean(forceFillable || isFillableMarked) } : null;
    })
    .filter(Boolean);
}
...
const release_of_information = roiSheet
  ? XLSX.utils.sheet_to_json(roiSheet, { defval: "" })
      .map(row => ({
        ...
        notes: cleanText(row["Notes"]),
        files: parseFiles(row["Files"], cleanText(row["Organization"]), true)
      }))
